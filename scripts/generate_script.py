...
# 注意（内容の正確さ・読み上げの自然さ）
- 「からっ風(からっかぜ)」は【冬（おおむね12〜2月）だけ】に吹く冷たく乾いた強い季節風です。
  春〜秋（おおむね3〜11月）は、風の意味での「からっ風」という話題・言葉を【基本的に出さない】
  でください。「からっ風が吹く/吹いている」「からっ風の季節」等はもちろん、比喩や導入としても
  春〜秋には使わないこと。季節の風・気候に触れるなら、その季節に合うもの（夏なら夕立や夏祭り、
  秋なら実りや紅葉など）にしてください。
  ※これは風の話題の制限です。パーソナリティ名「からっ風太郎」は年中いつでも呼んでOKです。
...
# 話題の選び方（重要）
- 【ポジティブな話題のみ】を扱う。明るい・前向きな話題だけにする。
  例: 地域のお祭り・イベント、新店/新商品/新スポット、受賞・達成・記録更新、季節の楽しみ、
  特産品やものづくり、人の頑張りや心温まる話題、観光・グルメ、地域の豆知識・歴史・文化など。
- 事故・事件・災害・訃報・不祥事などのネガティブな話題は【扱わない】。
- 天気・気温・猛暑・熱中症・寒さなどの「天気ネタ」は【基本的に出さない（原則ゼロ）】。
  必要でも挨拶のひとこと程度までとし、天気を1つの話題として扱わない。
- 【いろいろなテーマを扱う】こと。1つの話題に偏らず、6〜7個の異なるポジティブな話題を紹介する。
- その日のニュースだけで6〜7個のポジティブな話題が揃わないときは、群馬・埼玉の定番の魅力
  （食・温泉・観光・文化・歴史・ものづくり・方言あるある・地域の豆知識など）で補ってよい。

# 出力について
submit_script ツールを使って台本(セリフの配列)を提出してください。
オープニング挨拶→【6〜7個の異なるポジティブな話題を1つずつ、それぞれ数往復の掛け合いで紹介】
→エンディングの流れにしてください。1話題で終わらせず、テンポよく話題を移していきます。
全体で【80〜110セリフ程度、読み上げて6〜7分】になる分量にしてください。
...
そうなものを【できるだけ多め】に列挙してください。特に次は【必ず】拾うこと:
・人名(例: 布袋=ホテイ)、学校名・スポーツチーム名(例: 花咲徳栄=ハナサキトクハル、
  前橋育英=マエバシイクエイ)、企業・施設・イベント名などの固有名詞
・地名(例: 熊谷=クマガヤ)、難読漢字、複数の読み方がある語(例: 大人気=ダイニンキ)
・ふつうの熟語や言い回しでも、機械が読み違えやすいもの
  (例: 安全面=アンゼンメン、何ですか=ナンデスカ、何でしょう=ナンデショウ)。
  「なに/なん」「音読み/訓読み」が紛れる語は特に注意して正しい読みを入れる
各要素は surface(台本に出てくる表記) と kana(正しい読み・全角カタカナ) のペアにします。
固有名詞は「たぶん大丈夫」と思っても念のため入れてください。ただし読みが確実に分かるものだけを
書き、自信がない読みは推測で書かないこと(その語は台本上でカタカナ表記に置き換えてもよい)。
台本に出てこない語は含めないこと。
"""
...
MAX_LINES = 160  # 暴走生成のセーフガード(想定は80〜110セリフ程度)
...
def _call_claude(prompt):
    ...
    resp = client.messages.create(
        ...
        # 80〜110セリフ + episode_summary(約200字) + readings を1回のtool_useで返すため、
        # 出力が途中で切れて壊れた入力(raw多数/valid=0)にならないよう十分な余裕を持たせる。
        max_tokens=16000,
        tools=[SCRIPT_TOOL],
